...
def location_to_string(location):
    ...
    letter = chr(location[0] + 65)
    num = str(location[1] + 1)
    str_rep = letter + num
...
def is_within_board(location, direction):
    ...
    return is_legal_location(new_loc)
...
def all_possible_moves_for(player):
    ...
    for i in all_locations():
        if at(i) == player:
            for dir in possible_moves_from(i):
                all_moves.append((i, dir))
...
def make_move(location, direction):
    ...
    player = at(location)
    adj_loc = adjacent_location(location, direction)
    board[adj_loc[0]][adj_loc[1]] = player
...
def choose_computer_move(who):
    ...
    for loc in all_locations():
        if at(loc) == who:
            return all_possible_moves_for(at(loc))[0]
...
def is_enemy_win():
    ...
    def same_row(lst):
        return lst[0][0] == lst[1][0] and lst[1][0] == lst[2][0]

    def same_col(lst):
        return lst[0][1] == lst[1][1] and lst[1][1] == lst[2][1]

    return same_col(m_list) or same_row(m_list)
